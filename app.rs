...
use crate::compute::compute_approximation;
...
impl MyApp {
    pub fn new() -> Self {
        ...
        let client = rt.block_on(get_client()).expect("Gagal Konek ke MongoDB");
    ...
    fn draw_dashboard(&self, ui: &mut egui::Ui) {
        ui.heading("📈 Grafik Sensor (30 Data Terbaru)");

        let values: Vec<f64> = self.data.iter().map(|d| d.value).collect();
        let model_data = compute_approximation(&values);
        ...
        Plot::new("plot")
            ...
            .allow_zoom(true)
            .show(ui, |plot_ui| {
                plot_ui.line(Line::new(recent_points).name("Sensor Asli"));
                plot_ui.line(Line::new(model_data).name("Model Taylor").color(Color32::YELLOW));
            });
    }
